...
namespace Authentication.Shared.Models
{
    ...
    {
        public static async Task<Profile> GetById(string id)
        {
            var query = new QueryDefinition("select * from c where c.id = @id").WithParameter("@id", id);
            var result = await DataService.Instance.QueryDocuments<Profile>("Profile", query);
        ...
        public static async Task<List<Profile>> GetByUserId(string userId)
        {
            ...
            return await DataService.Instance.QueryDocuments<Profile>("Profile", query);
        }
    }
}